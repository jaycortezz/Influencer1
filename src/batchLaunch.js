...
function parseArgs(argv) {
  const args = { concurrency: 3, out: "output", download: true, dryRun: false };
  const positional = [];
...
async function main() {
  ...
  if (!args.batchFile) {
    console.error("Usage: node src/batchLaunch.js <batch-file.json> [--concurrency N] [--out DIR] [--model MODEL] [--no-download] [--dry-run]");
    process.exit(1);
  }
  ...
  const results = await runPool(jobs, args.concurrency, async (job, index) => {
    const input = { ...defaults, ...job };
